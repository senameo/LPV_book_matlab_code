...
switch scenario
    case 1
sim('Simulations_chapter5_method1_3scenarios')
Estimation_error=Fd_est.signals.values-Fd_r.signals.values;
...
subplot(2,2,3), plot(Fd_est.time,Fd_r.signals.values,Fd_est.time,Fd_est.signals.values,'r'), title('Force and Estimated force')
...
disp('NRMSE')
NRMSE=rms(Estimation_error)/(max(Fd_r.signals.values)-min(Fd_r.signals.values))
...
% 
% figure, 
% subplot(2,2,1), plot(u.time,u.signals.values-mean(u.signals.values), zr.time,zr.signals.values),title('control signal u & road profile zr')
% subplot(2,2,2), plot(rho.time,rho.signals.values), title('scheduling parameter') 
% subplot(2,2,3), plot(Fd_est.time,Fd_r.signals.values,Fd_est.time,Fd_est.signals.values,'r'), title('Force and Estimated force')
% subplot(2,2,4), plot(Fd_est.time,Estimation_error), title('Estimation error')
% sgtitle('Scenario 3')
